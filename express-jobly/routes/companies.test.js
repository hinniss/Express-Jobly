...
describe("GET /companies", function () {
  ...
  test("ok with name filter", async function () {
  ...
  test("ok with min and max employees filters", async function () {
  ...
  test("fails: error if minEmployees greater than maxEmployees", async function () {
    const resp = await request(app).get("/companies?minEmployees=3&maxEmployees=2");
    expect(resp.statusCode).toBe(400);
    expect(resp.body).toEqual({
      "error": {
        "message": 'minEmployees must be greater than or equal to maxEmployees',
        "status": 400
      }
    });
  });
  ...
  test("fails: error if additional query parameters provided", async function () {
    const resp = await request(app).get("/companies?minEmployees=3&maxEmployees=2&other=6");
    expect(resp.statusCode).toBe(400);
    expect(resp.body).toEqual({
      "error": {
